...
export const MiniMap = ({ data }: { data: PreviewManager["data"] }) => {
  ...
  const S = 0.1
  const MW = CANVAS_W * S, MH = CANVAS_H * S
  const mx = CANVAS_W - MW - 8, my = CANVAS_H - MH - 8
  ...
  return (
    <g>
      ...
      {data.tokens.map((token) => {
        ...
        if (!pt) return null
        const dx = mx + pt.x * S, dy = my + pt.y * S
        const color = (token.displayColor || token.color) as string
        return token.type === "square"
          ? <rect key={token.id} x={dx - 3} y={dy - 3} width={6} height={6} fill={color} stroke="#000" strokeWidth={1}/>
