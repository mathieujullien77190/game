# @drift/app — jeu Drift en React Native (Skia)

Rend **exactement** la preview web, via `SkiaRenderer` (@drift/skia-render) qui implémente
l'interface `Renderer` de l'engine. Sim + draw = 100% réutilisés depuis `@drift/engine`.
...
`@drift/skia-render` typecheck contre les vrais types `@shopify/react-native-skia` (OK).
...
- **Metro monorepo** : `metro.config.js` active `unstable_enablePackageExports` pour résoudre
  les subpaths `@drift/*` (source TS). Si un `@drift/...` ne résout pas, vérifier ce flag.
- **map.json** : importé statique depuis `@drift/maps`. Le seed est vide → écran blanc tant
