...
const lineMid = (line: LinePreview) => {
  if (line.points.length === 0) return { x: (line.start.x + line.end.x) / 2, y: (line.start.y + line.end.y) / 2 }
...
export class LinePreview extends Line {
  ...
  render = (elapsed: number): JSX.Element => {
    const {
      BOOST_WIN_RATIO, BOOST_ANIM_SPEED, BOOST_GLOW_COLOR, BOOST_STROKE_WIDTH,
      TUNNEL_DOT_R, LINE_STROKE, LINE_STROKE_WIDTH,
    } = LinePreview
    const boostGlow = (() => {
      ...
      return (
        <path d={d} fill="none" stroke={BOOST_GLOW_COLOR} strokeWidth={BOOST_STROKE_WIDTH} strokeLinecap="round"
  ...
  renderOverlay = (token?: TokenPreview): JSX.Element | null => {
    ...
    const {
      SPEED_BOX_W, SPEED_BOX_H, SPEED_BOX_RX, SPEED_FONT_SIZE,
      LIMIT_R, LIMIT_OFFSET_X,
    } = LinePreview
