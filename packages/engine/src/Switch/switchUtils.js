...
exports.curveIntersectAngle = exports.getSwitchEnterPoint = void 0;
const getSwitchEnterPoint = (linkIds, links) => {
    if (linkIds.length === 0)
        return null;
    const first = links[linkIds[0]];
    if (!first)
        return null;
    let candidates = [{ ...first.line1 }, { ...first.line2 }];
    for (const lkId of linkIds.slice(1)) {
        const lk = links[lkId];
        if (!lk)
            continue;
        candidates = candidates.filter((c) => (lk.line1.lineId === c.lineId && lk.line1.endpoint === c.endpoint) ||
            (lk.line2.lineId === c.lineId && lk.line2.endpoint === c.endpoint));
    }
    return candidates[0] ?? null;
};
exports.getSwitchEnterPoint = getSwitchEnterPoint;
const curveIntersectAngle = (pts, endpointSide, cx, cy, r) => {
...
exports.curveIntersectAngle = curveIntersectAngle;
