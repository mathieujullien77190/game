...
- **Mobile (`app`)** : pas de rendu natif. `App.tsx` = une `<WebView>` (`react-native-webview`) qui charge `src/previewHtml.ts` — un HTML single-file (build Vite de `webview/`, cf. `scripts/build-preview.mjs`) contenant engine + Canvas2DRenderer + la boucle rAF. Les niveaux sont passés via `injectedJavaScriptBeforeContentLoaded` (`window.__TICTACTIC_LEVELS__ = [{ id, file, json }]`). **Niveaux = tous les `packages/maps/map*.json`**, dans l'ordre naturel (map, map2, map3… map10) — règle unique dans `packages/maps/mapFiles.mjs`. Le plugin dev de `webview/vite.config.ts` lit le dossier directement ; `App.tsx` (Metro, imports statiques uniquement) importe l'index **généré** `packages/maps/levels.gen.ts`, régénéré par l'éditeur à chaque sauvegarde de map et avant `yarn app` / `app:start` (`gen:levels`). Aucune liste à tenir à la main → changer une map ne nécessite **pas** de rebuild le bundle web (juste reload l'app). Rebuild `build:preview` quand l'engine / le renderer / `webview/` changent.
- **UI du jeu = charte « Calque »** (Blueprint D clair — maquette : `design/charte-graphique/blueprint/D-calque.html` ; les écrans `design/charte-graphique/cao/` servent de référence de structure) en DOM pur, pas de React. Markup + CSS des 4 écrans dans `webview/index.html` (sections `#s-home`, `#s-levels`, `#s-game`, `#s-options`), logique dans `webview/ui/` : `router` (un écran visible), `home`, `levelsScreen` (liste des niveaux à la suite, sans aperçu), `game` (boucle rAF, HUD, journal, dialogues pause / victoire / échec), `optionsScreen`, `storage` (options + progression en localStorage), `theme` (palette Calque appliquée à l'engine via `applyTheme`).
- **Victoire / échec** décidés par `webview/ui/game.ts` (pas l'engine) : victoire = toutes les demandes des arrivals livrées ; échec = mauvaise balle à une arrivée, collision, ou plus aucune balle en mouvement (impasse). Map sans demandes = bac à sable (pas d'issue). S'appuie sur les événements de simulation (`pm.drainEvents()`).
...
- **app** (mobile) : `App.tsx` importe `map.json` + `map2.json` (Metro) et les injecte comme niveaux dans la WebView → `buildPreviewManager(json)` par niveau (cf. section Rendering).
...
- `yarn app:web` → visu web de l'app dans le navigateur (Vite dev sur `app/webview`, même bundle que la WebView). Niveaux injectés depuis `maps/*.json` (plugin dev `tic-tac-tic-inject-levels`), page rechargée à chaque modif d'une map
- `yarn build:web` → version web **autonome** du jeu (`packages/app/webview/dist-web/index.html`, un seul fichier, niveaux `maps/map*.json` inlinés au build via `--mode web`). Déployée sur **GitHub Pages** à chaque push sur `main` (`.github/workflows/pages.yml`) → intégrable en iframe (portfolio)
